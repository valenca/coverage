...
trans={"KD"		:{"10":"#00CDCD","15":"#388E8E","20":"#2F4F4F"},
	   ...
	   "BP15"	:{"10":"#CC00FF","15":"#9A32CD","20":"#4B0082"},
	...
	for directory in product(lista,files):
		...
		if directory[0]!='KD':
			...
			plt.plot(range(1000,31000,1000),
					 ...
					 linewidth=2,color=trans[directory[0]])
			labels.append(names[directory[0]]+" "+t)
			if directory[1]=='results_unif20':
				print np.mean(arr,axis=1)[29]
		else:
			with open('LS'+"/res/"+directory[1]) as f:
				for j in range(30):
					v=[]
					for i in range(30):
						v.append(float(f.readline().split()[4]))
