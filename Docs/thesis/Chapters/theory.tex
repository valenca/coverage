...
\noindent
\paragraph{}
In Figure \ref{fig:gr1}, the search for the closest vertex to $p$ starts at point $v$. From there, point $u$, which is closer to $p$ than $v$ is, is found. The step repeats following the blue path until point $w$ is reached. Since no neighbour of $w$ is closer to $p$ than $w$ is, then $p$ must be within $w$'s Voronoi cell, shaded light grey.
\begin{theorem}
\cite{greedyroute}
